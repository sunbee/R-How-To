...
w$currently$summary
```
The forecast is *Mostly Cloudy* for that day. What if I have a set of lat-long coordinates with time-stamps in a table that I want to fetch weather data for? 
...
I can loop over the rows in the table. A more elegant solution is to vectorize the function.
...
```
The function is now ready to accept lists as inputs. In general, the output is a list of the same size as the input. 
```{r}
